...
import { Search, BookOpen, Award, Trophy, Star, BookmarkCheck, CheckCircle2, Zap, Medal, PartyPopper, AlertTriangle, BookX, Beaker, Lightbulb, Timer, ListChecks, ShoppingCart, Clipboard, GraduationCap, FlaskConical } from "lucide-react";
const LearningModulePage = ({ pathId }) => {
    ...
    const [pathQuizzes, setPathQuizzes] = useState([]);
    ...
    const checkAnswer = () => {
        if (!selectedOption || isAnswerChecked)
            return;
    ...
    if (!currentPath || !currentQuiz) {
        return (<div className="pt-16 flex justify-center items-center min-h-screen">
          ...
          <div className="lg:w-1/4 mb-8 lg:mb-0 lg:pr-6">
            ...
            <div className="bg-white rounded-xl shadow-md p-6 mb-6">
              <div className="relative">
                ...
                <Input type="search" placeholder="Search modules..." className="pl-10 focus:border-primary" value={searchQuery} onChange={handleSearch}/>
              </div>
            </div>
            ...
            return (<div key={module.id} className="relative">
                      <div className="flex items-start">
                        <div className="flex flex-col items-center">
                          <div className={`flex items-center justify-center h-8 w-8 rounded-full ${isCompleted ? 'bg-[#28C76F] text-white' :
                    isCurrent ? 'bg-primary text-white font-medium' :
                        'bg-neutral-300 text-neutral-700 font-medium'}`}>
                            {isCompleted ? (<i className="ri-check-line"></i>) : (module.order)}
                          </div>
                          {module.order < totalModules && (<div className={`h-full w-0.5 ${isCompleted ? 'bg-[#28C76F]' : 'bg-neutral-300'}`}></div>)}
                        </div>
                        <div className="ml-4">
                          ...
                          {isCompleted ? (<span className="text-xs text-[#28C76F] font-medium">Completed</span>) : isCurrent ? (<>
            ...
            <div className="bg-white rounded-xl shadow-md p-6 mt-6">
              ...
              <div className="space-y-4">
                {achievements.map((achievement, index) => (<div key={index} className={`flex items-center p-3 rounded-lg transition-all ${unlockedAchievements[index]
                ? `bg-${achievement.color}-50 hover:bg-${achievement.color}-100`
                : 'bg-neutral-100 text-neutral-400'}`}>
                    <div className={`flex items-center justify-center w-10 h-10 rounded-full ${unlockedAchievements[index]
                ...
                : 'bg-neutral-200 text-neutral-400'}`}>
                    ...
                    {unlockedAchievements[index] && (<div className="ml-auto">
              ...
              <Tabs defaultValue="concept" className="w-full">
                ...
                <TabsContent value="concept" className="mt-0">
                  <div className="max-w-2xl mx-auto">
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="bg-neutral-50 rounded-xl p-6">
                      ...
                      <ul className="space-y-2 mb-6">
                        {currentModule?.learningObjectives?.map((objective, index) => (<li key={index} className="flex items-start">
                      ...
                      <div className="flex flex-wrap gap-2 mb-6">
                        {currentModule?.keyTerms?.map((term, index) => (<Badge key={index} variant="outline" className="bg-primary/10 text-primary border-primary/20 px-3 py-1">
                            {term}
                          </Badge>))}
                      </div>
                ...
                <TabsContent value="quiz" className="mt-0">
                  <div className="max-w-2xl mx-auto">
                    {/* Question Card */}
                    <motion.div key={currentQuiz.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="bg-neutral-50 rounded-xl p-6">
                      <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="space-y-3">
                        {currentQuiz.options.map((option) => (<QuizOption key={option.id} option={option} selectedOption={selectedOption} correctOption={currentQuiz.correctOption} isAnswerChecked={isAnswerChecked} onSelect={() => handleOptionSelect(option.id)}/>))}
                      </div>

                      {/* Check Answer Button */}
                      {selectedOption && !isAnswerChecked && (<div className="mt-6">
                          <button className="w-full bg-primary text-white font-medium py-3 rounded-lg shadow-sm hover:bg-opacity-90 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2" onClick={checkAnswer}>
                            Check Answer
                          </button>
                        </div>)}

                      {/* Explanation (Hidden initially) */}
                      {isAnswerChecked && (<div className="mt-6 bg-neutral-100 rounded-lg p-4">
                          <h4 className="font-medium text-neutral-900">Explanation</h4>
                          <p className="text-neutral-700 mt-1">
                            {currentQuiz.explanation}
                          </p>
                          <div className="mt-4">
                            <button className="w-full bg-primary text-white font-medium py-3 rounded-lg shadow-sm hover:bg-opacity-90 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2" onClick={nextQuestion}>
                              Continue
                            </button>
                          </div>
                        </div>)}
                ...
                <TabsContent value="stem" className="mt-0">
                  <div className="space-y-8">
                    ...
                    {currentModule?.stemActivities && currentModule.stemActivities.length > 0 ? (<div className="space-y-6">
                        {currentModule.stemActivities.map((activity) => (<Card key={activity.id} className="overflow-hidden">
                            <CardHeader className="bg-gradient-to-r from-purple-50 to-blue-50 pb-2">
                              <div className="flex justify-between items-start">
                                ...
                                <Badge variant="outline" className={activity.difficultyLevel === "beginner" ? "bg-green-100 text-green-800 border-green-200" :
                                ...
                                <div>
                                  ...
                                  <div className="bg-neutral-50 rounded-lg p-4 mb-4">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                      {activity.materials.map((material, index) => (<Badge key={index} variant="outline" className="bg-white">
                                          {material}
                                        </Badge>))}
                                    </div>
                                  </div>
                                  ...
                                  <div className="mb-4">
                                    ...
                                    <ol className="list-decimal pl-5 space-y-1">
                                      {activity.steps.map((step, index) => (<li key={index} className="text-sm text-neutral-700">{step}</li>))}
                                    </ol>
                                  </div>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-center text-xl">
              {completionPassed ? (<div className="flex items-center justify-center gap-2 text-[#28C76F]">
                  <PartyPopper className="h-6 w-6"/>
                  <span>Congratulations!</span>
                </div>) : (<div className="flex items-center justify-center gap-2 text-amber-500">
                  <AlertTriangle className="h-6 w-6"/>
                  <span>Module Review Required</span>
                </div>)}
            </DialogTitle>
            <DialogDescription className="text-center pt-2">
              {completionPassed ? (<span>You've successfully completed the {currentModule?.title} module!</span>) : (<span>You'll need to review the material and try again.</span>)}
            </DialogDescription>
          </DialogHeader>
          
          <div className="p-5">
            <div className="mb-6 flex justify-center">
              {completionPassed ? (<div className="flex items-center justify-center w-24 h-24 rounded-full bg-[#28C76F]/10 text-[#28C76F]">
                  <Medal className="h-12 w-12"/>
                </div>) : (<div className="flex items-center justify-center w-24 h-24 rounded-full bg-amber-100 text-amber-500">
            ...
            : "Learning takes time. Review the material, take notes, and try the quiz again."}
              ...
              <div className="bg-neutral-100 p-3 rounded-lg mb-4">
                ...
                {completionPassed && (<div className="flex justify-between items-center">
